...
Many common basis sets in quantum chemistry are the Gaussian related basis set, since molecular integral w.r.t these basis functions is easy to evaluate. Let's start first from Cartesian Gaussian type orbital (GTO):
...
$$\begin{eqnarray}
G_{ijk}(\vec{x};\vec{A}) & = & N_{ijk}\frac{\factorial{i}\factorial{j}\fractorial{k}}{2^{i+j+k}}\sum_{i^{\prime},j^{\prime},k^{\prime}=0}^{\[0.5i,0.5j,0.5k\]}\\
& & \underbrace{\frac{1}{\factorial{i^{\prime}}\factorial{i-2i^{\prime}}\alpha^{i-i^{\prime}}}\times \frac{1}{\factorial{j^{\prime}}\factorial{j-2j^{\prime}}\alpha^{j-j^{\prime}}} \times \frac{1}{\factorial{k^{\prime}}\factorial{k-2k^{\prime}}\alpha^{k-k^{\prime}}}}_{F_{ijk}^{i^{\prime}j^{\prime}k^{\prime}}(\alpha)}\\
...
\text{Note:} & & x^n=\frac{\factorial{n}}{2^n}\sum_{i=0}^{\[0.5n\]}\frac{H_{n-2i}(x)}{\factorial{i}\factorial{n-2i}}\\
